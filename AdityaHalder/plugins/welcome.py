...
from AdityaHalder.modules.helpers.program import get_arg, welcome_chat
import AdityaHalder.modules.databases.welcome_db as Kaal
from AdityaHalder.config import LOG_GROUP_ID
...
async def new_welcome(app: Client, message):
    msg_id = await Kaal.get_welcome(str(message.chat.id))
    caption = ""
    men = ""
...
async def setwelcome(app: Client, message):
    ...
    await Kaal.save_welcome(str(message.chat.id), msg_id)
